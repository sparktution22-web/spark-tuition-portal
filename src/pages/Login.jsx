...
export default function Login() {
  ...
  const [loginMode, setLoginMode] = useState('family') // 'family' | 'admin'
  ...
  return (
    <div className="min-h-screen bg-spark-surface flex items-center justify-center px-5 py-12 relative overflow-hidden">
      ...
      >
        ...
        {!isMockAuth && (
          <div className="flex justify-center gap-1 mb-6 bg-spark-surface rounded-full p-1 text-xs font-semibold">
            <button
              type="button"
              onClick={() => setLoginMode('family')}
              className={`px-4 py-1.5 rounded-full transition-colors ${
                loginMode === 'family' ? 'bg-white shadow-soft text-spark-orange' : 'text-spark-ink/50'
              }`}
            >
              Parent / Student
            </button>
            <button
              ...
              className={`px-4 py-1.5 rounded-full transition-colors ${
                loginMode === 'admin' ? 'bg-white shadow-soft text-spark-orange' : 'text-spark-ink/50'
              }`}
            >
              Admin
            </button>
