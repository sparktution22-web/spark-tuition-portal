...
import { getTests, createTest, fileToBase64 } from '../../services/api/sheetsApi.js'
...
export default function AdminTests() {
  ...
  const submit = async (e) => {
    ...
    setError('')
    setSuccess('')
    if (!subject.trim() || !testName.trim() || !maxMarks || !file) {
      setError('All fields, including the question paper PDF, are required.')
      return
    }
    ...
    try {
      ...
      await createTest({ subject: subject.trim(), testName: testName.trim(), maxMarks: Number(maxMarks), questionPaperBase64: base64 })
      setSuccess(`"${testName}" created — students can now submit answers against it.`)
      setSubject('')
      setTestName('')
      setMaxMarks('')
      setFile(null)
            ...
            {tests.map((t) => (
              <div key={t.testId} className="flex items-center justify-between border-b border-spark-ink/5 dark:border-white/10 last:border-0 pb-3 last:pb-0">
                <div>
                  ...
                  <p className="text-xs text-spark-ink/40 dark:text-white/40">{t.subject}</p>
                </div>
