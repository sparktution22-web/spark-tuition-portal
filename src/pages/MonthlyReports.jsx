...
import { FiDownload, FiFileText, FiCalendar, FiSend, FiExternalLink, FiImage } from 'react-icons/fi'
...
import { generateMonthlyReportImage } from '../../utils/imageGenerator.js'
...
export default function MonthlyReports() {
  ...
  // Shared by handleDownload / handleSendToParent / handleExportImage:
  // makes sure we're working with THIS month's data even if the button
  // clicked isn't the currently-selected month in the preview card above.
  const ensureMonthData = async (monthKey) => {
  ...
  const handleDownload = async (monthLabel, monthKey) => {
    ...
    setGenerating(true)
    try {
      const { info: reportInfo, attendance: reportAttendance, marks: reportMarks } = await ensureMonthData(monthKey)
  ...
  // WhatsApp-shareable image export — a compact PNG "report card"
  // instead of the full PDF. Unlike a PDF attachment, an image previews
  // inline in a WhatsApp chat, which some parents find easier to glance
  // at on mobile. Downloads locally so it can be attached in WhatsApp
  // the same way the PDF download already works — no backend changes
  // needed, since this never touches Code.gs.
  const handleExportImage = async (monthLabel, monthKey) => {
    if (!selectedStudent) return
    setExportingMonth(monthKey)
    setMonthError('')
    try {
      const { info: reportInfo, attendance: reportAttendance, marks: reportMarks } = await ensureMonthData(monthKey)
      await generateMonthlyReportImage({
        student: { ...selectedStudent, ...reportInfo },
        attendance: reportAttendance,
        marks: reportMarks,
        monthLabel
      ...
      setMonthError(err.message || 'Could not generate the image. Please try again.')
    } finally {
      setExportingMonth('')
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-white/5 rounded-xl2 shadow-card border border-spark-ink/5 dark:border-white/10 p-6">
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={generating || sendingMonth === m.key || exportingMonth === m.key}
                ...
                <button
                  onClick={() => handleExportImage(m.label, m.key)}
                  disabled={generating || sendingMonth === m.key || exportingMonth === m.key}
                  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-white dark:bg-white/10 border-2 border-spark-orange text-spark-orange text-xs font-bold hover:bg-spark-orange hover:text-white transition-all disabled:opacity-60"
                >
                  <FiImage size={13} /> {exportingMonth === m.key ? 'Rendering...' : 'Image'}
                </button>
              </div>
              <button
                onClick={() => handleSendToParent(m.label, m.key)}
                disabled={generating || sendingMonth === m.key || exportingMonth === m.key}
                className="w-full mt-2 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-500 text-white text-xs font-bold shadow-soft hover:shadow-card-hover transition-all disabled:opacity-60"
              >
        ...
        <p className="text-xs text-spark-ink/40 dark:text-white/40 mt-4">
          "Image" downloads a compact PNG version of this report — handy for attaching directly in WhatsApp, where it shows as an inline preview instead of a document to open.
        </p>
        <p className="text-xs text-spark-ink/40 dark:text-white/40 mt-1">
          "Send to Parent" saves the PDF report, notifies the family in-app, and opens WhatsApp with a link to it — only sends after you tap Send in WhatsApp yourself.
        </p>
        <p className="text-xs text-spark-ink/40 dark:text-white/40 mt-1">
