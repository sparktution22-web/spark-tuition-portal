...
export default function TestMarks() {
  ...
  const overall = marks.length ? Math.round(marks.reduce((s, m) => s + m.score, 0) / marks.length) : 0
  const highest = marks.length ? Math.max(...marks.map((m) => m.score)) : 0
  const highestSubject = marks.find((m) => m.score === highest)?.subject || '-'
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-white/5 rounded-xl2 shadow-card border border-spark-ink/5 dark:border-white/10 overflow-hidden">
        <h3 className="font-display font-bold text-spark-ink dark:text-white p-6 pb-4">
          Subject-wise Marks <span className="text-spark-orange text-sm font-semibold ml-2">Highest in {highestSubject}</span>
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {marks.map((m) => (
                <tr key={m.subject} className="border-t border-spark-ink/5 dark:border-white/5">
                  ...
                  <td className="px-6 py-3.5">
                    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-spark-peach text-spark-orange">
                      {gradeFromPercent(m.score)}
                    </span>
                  </td>
                  <td className="px-6 py-3.5">
                    <div className="h-2 rounded-full bg-spark-peach dark:bg-white/10 overflow-hidden">
                      <div className="h-full bg-spark-gradient rounded-full" style={{ width: `${m.score}%` }} />
                    </div>
                  </td>
                </tr>
              ))}
